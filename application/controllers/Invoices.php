...
class Invoices extends CI_Controller
{
    ...
    public function salvar_capa()
    {
        ...
        if ($data_emissao > date("Y-m-d")) {
            die(
                "<script>alert('ERRO: Data de emissão não pode ser futura!'); history.back();</script>"
            );
        }

        $dados_nota = [
            "numero_nota" => preg_replace(
                "/[^0-9]/",
                "",
                $this->input->post("numero_nota")
            ),
    ...
    public function salvar_item_unitario()
    {
        ...
        $produto = $this->db
            ->get_where("catalogo_produtos", ["id" => $id_prod])
            ->row();
    ...
    public function detalhes($id)
    {
        ...
        $this->db->where(
            "e.id_invoice_item IN (SELECT id FROM invoice_items WHERE id_invoice = " .
                $id .
                ")"
        );
    ...
    public function confirmar_alocacao()
    {
        ...
        if (
            $this->Estoque_model->verificar_posicao_ocupada(
                $rua,
                $posicao,
                $id_catalogo,
                $lote
            ...
            die(
                "<script>alert('ERRO: A RUA $rua - POS $posicao já está ocupada por outro produto ou lote!'); history.back();</script>"
            );
        }
        ...
        if (
            $this->Estoque_model->alocar_no_estoque(
                $data_estoque,
                $data_estoque["id_invoice_item"],
                $data_estoque["quantidade"]
            )
        ) {
    ...
    public function deletar($id_invoice)
    {
        $this->db->trans_start();
        $itens = $this->db
            ->get_where("invoice_items", ["id_invoice" => $id_invoice])
            ->result();
        ...
        foreach ($itens as $item) {
            if ($this->Estoque_model->item_possui_saida($item->id)) {
                ...
                die(
                    "<script>alert('BLOQUEIO: Esta nota não pode ser deletada pois itens dela já saíram em requisições!'); history.back();</script>"
                );
            }
            $this->db
                ->where("id_invoice_item", $item->id)
                ->delete("estoque_v2");
        }
    ...
    {
        $alocacao = $this->db
            ->get_where("estoque_v2", ["id" => $id_estoque])
            ->row();

        if ($alocacao) {
            if (
                $this->Estoque_model->item_possui_saida(
                    $alocacao->id_invoice_item
                )
            ) {
                die(
                    "<script>alert('ERRO: Não é possível remover pois este item já possui saídas registradas!'); history.back();</script>"
                );
            }
            ...
            $this->db->set(
                "quantidade_alocada",
                "quantidade_alocada - " . (float) $alocacao->quantidade,
                false
            );
