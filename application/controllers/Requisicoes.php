...
class Requisicoes extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $dados[
            "requisicoes"
        ] = $this->Estoque_model->listar_requisicoes_filtradas($filtros);
    ...
    public function estornar($id_requisicao)
    {
        ...
        $req = $this->db
            ->get_where("requisicoes", ["id" => $id_requisicao])
            ->row();

        // Só permite estornar se estiver em picking ou finalizada
        if (
            !$req ||
            ($req->status != "PICKING" && $req->status != "FINALIZADO")
        ) {
            die(
                "<script>alert('ERRO: Esta requisição não pode ser estornada no status atual: " .
                    ($req->status ?? "N/A") .
                    "'); history.back();</script>"
            );
        }
        ...
        $this->db
            ...
            ->update("requisicoes", ["status" => "ABERTO"]);
        ...
        if ($this->db->trans_status() === false) {
            $this->session->set_flashdata(
                "erro",
                "Erro crítico ao processar estorno."
            );
        } else {
            $this->session->set_flashdata(
                "sucesso",
                "Estorno realizado com sucesso! Saldo devolvido ao estoque."
            );
        }
    ...
    public function salvar()
    {
        ...
        if (!$id_cliente || empty($itens)) {
            $this->session->set_flashdata(
                "erro",
                "ERRO: Selecione um cliente e ao menos um item!"
            );
        ...
        if ($this->db->trans_status() === false) {
            $this->session->set_flashdata(
                "erro",
                "Erro ao gravar requisição no banco."
            );
            redirect("requisicoes/nova");
        } else {
            $this->session->set_flashdata(
                "sucesso",
                "Requisição #" . $id_requisicao . " criada com sucesso!"
            );
    ...
    public function confirmar_picking($id_requisicao)
    {
        ...
        foreach ($itens_pedidos as $item) {
            ...
            foreach ($posicoes as $pos) {
                ...
                $nova_qtd_posicao =
                    (float) $pos->quantidade - (float) $quantidade_retirar;
            ...
            if ($quantidade_falta > 0) {
                ...
                die(
                    "ERRO: Saldo insuficiente total para " .
                        $item->nome_produto .
                        ". Faltam: " .
                        $quantidade_falta
                );
        ...
        $this->session->set_flashdata(
            "sucesso",
            "Picking confirmado! Quantidades deduzidas de múltiplas posições."
        );
        redirect("requisicoes/detalhes/" . $id_requisicao);
    }
    //
