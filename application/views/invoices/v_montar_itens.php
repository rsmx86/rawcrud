<table width="100%" border="1" cellspacing="0" cellpadding="5" bgcolor="#C0C0C0" bordercolorlight="#FFFFFF" bordercolordark="#808080">
    <tr>
        ...
        <td width="25%"><font size="1">DATA EMISSÃO:</font><br><b><?= date(
            "d/m/Y",
            strtotime($invoice->data_emissao)
        ) ?></b></td>
        <td width="25%" bgcolor="#E1E1E1"><font size="1">VALOR ALVO (NF):</font><br><font color="blue"><b>R$ <?= number_format(
            ...
            <form action="<?= site_url(
                ...
                <select name="id_catalogo" required style="width: 40%;">
                    ...
                    <?php foreach ($produtos_catalogo as $p): ?>
                        <option value="<?= $p->id ?>"><?= $p->nome_produto ?> (REF: R$ <?= number_format(
     $p->valor_unitario,
     2,
     ",",
     "."
...
<table width="100%" border="1" cellspacing="0" bgcolor="#FFFFFF" bordercolorlight="#FFFFFF" bordercolordark="#808080">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td align="right"><font size="2">R$ <?= number_format(
                $i->valor_unitario,
                2,
                ",",
                "."
            ) ?></font></td>
            <td align="right"><font size="2"><b>R$ <?= number_format(
                $i->valor_total_item,
                2,
                ",",
                "."
            ...
            <td align="center" bgcolor="#F0F0F0">
                <a href="<?= site_url(
                    "invoices/remover_item/" . $i->id . "/" . $invoice->id
                ) ?>" 
        ...
        <td align="right">
            <div style="border: 2px dashed #808080; padding: 10px; background: #EEE;">
                <font face="Arial" size="2">
                    VALOR TOTAL DA NF: <b>R$ <?= number_format(
                        $invoice->valor_total_nota,
                        2,
                        ",",
                        "."
                    ) ?></b><br>
                    SOMA DOS ITENS: <b>R$ <?= number_format(
                        $total_acumulado,
                        2,
                        ",",
                        "."
                ...
                <hr>
                <?php if (
                    round($total_acumulado, 2) ==
                    round($invoice->valor_total_nota, 2)
                ): ?>
                    <button onclick="window.location.href='<?= site_url(
                        "invoices/detalhes/" . $invoice->id
                    ) ?>'" 
