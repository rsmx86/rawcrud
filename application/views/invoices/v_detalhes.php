...
<table width="95%" border="2" cellspacing="0" cellpadding="2" bgcolor="#000000">
        ...
        <td colspan="2" bgcolor="#D6D2C4" style="padding:15px;">
            ...
            <table width="100%" border="1" cellspacing="0" cellpadding="5" bgcolor="#C0C0C0" bordercolorlight="#FFFFFF" bordercolordark="#808080">
                <tr>
                    ...
                    <td width="20%"><font size="1">DATA EMISSÃO:</font><br><b><?= date('d/m/Y', strtotime($invoice->data_emissao)); ?></b></td>
                    <td width="20%" bgcolor="#E1E1E1"><font size="1">VALOR TOTAL NOTA:</font><br><b>R$ <?= number_format($invoice->valor_total_nota, 2, ',', '.'); ?></b></td>
            ...
            ?>
            <div align="right">
                <font face="Arial" size="1">Total acumulado nos itens: <b>R$ <?= number_format($total_acumulado, 2, ',', '.'); ?></b></font><br>
                <font face="Arial" size="1" color="<?= $cor_status; ?>">Diferença p/ fechar nota: <b>R$ <?= number_format($diferenca, 2, ',', '.'); ?></b></font>
            </div>
                    ...
                    <?php else: ?>
                        ...
                        <tr <?php if($saldo <= 0) echo 'bgcolor="#E8FFE8"'; ?>>
                            ...
                            <td align="center" bgcolor="#C0C0C0">
                                <?php if($saldo > 0): ?>
                                    <form action="<?= site_url('invoices/confirmar_alocacao'); ?>" method="post" style="margin:0; display: flex; gap: 2px;">
    <input type="hidden" name="id_invoice" value="<?= $invoice->id; ?>">
    ...
    <button type="submit" style="font-size:10px; font-weight:bold; cursor:pointer; background:#C0C0C0;">[ OK ]</button>
            ...
            <table width="100%" border="0">
                <tr>
                    <td width="20%">
                        <table width="100%" border="2" cellspacing="0" cellpadding="5" bgcolor="#C0C0C0" bordercolorlight="#FFFFFF" bordercolordark="#808080" style="cursor: pointer;" onclick="window.location.href='<?= site_url('estoque'); ?>'">
...
<table width="100%" border="1" cellspacing="0" cellpadding="4" bgcolor="#F0F0F0" bordercolorlight="#FFFFFF" bordercolordark="#808080">
    ...
    <?php 
    // Supondo que você carregue as alocações da nota no controller
    ...
    <tr>
        ...
        <td align="right"><font size="2"><?= number_format($aloc->quantidade, 2, ',', '.'); ?></font></td>
        <td align="center">
            <a href="<?= site_url('invoices/remover_alocacao/'.$aloc->id.'/'.$invoice->id); ?>" 
