...
$action = $is_edit
    ? site_url("clientes/atualizar/" . $cliente->id)
    : site_url("clientes/salvar");
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="window">
    <div class="title-bar">
        ...
        <div style="cursor:pointer" onclick="window.location='<?= site_url(
            "clientes"
        ) ?>'">[X]</div>
    </div>
    ...
    <div class="content">
        <form action="<?= $action ?>" method="POST">
            ...
            <fieldset>
                ...
                <label>Full Name:</label> <input type="text" name="nome_completo" style="width: 350px;" value="<?= $is_edit
                    ? $cliente->nome_completo
                    : "" ?>" required><br>
                <label>Nickname/Nick:</label> <input type="text" name="nick" style="width: 150px;" value="<?= $is_edit
                    ? $cliente->nick
                    : "" ?>">
                <label>Phone Line:</label> <input type="text" name="telefone" id="telefone" value="<?= $is_edit
                    ? $cliente->telefone
                ...
                <select name="pais" id="pais" onchange="limparEndereco()">
                    <option value="Brasil" <?= $is_edit &&
                    $cliente->pais == "Brasil"
                        ? "selected"
                        : "" ?>>Brazil (BR)</option>
                    <option value="Japan" <?= $is_edit &&
                    $cliente->pais == "Japan"
                        ? "selected"
                        : "" ?>>Japan (JP)</option>
                </select>
                <label style="width: 70px; margin-left: 20px;">Zip/CEP:</label> 
                <input type="text" name="cep" id="cep" onblur="buscarEndereco()" style="width: 100px;" value="<?= $is_edit
                    ? $cliente->cep
                    : "" ?>">
                <br>
                
                <label>City/Prefecture:</label> <input type="text" name="cidade_provincia" id="cidade_provincia" style="width: 250px;" value="<?= $is_edit
                    ? $cliente->cidade_provincia
                    : "" ?>">
                <label style="width: 80px; margin-left: 10px;">District:</label> <input type="text" name="bairro" id="bairro" style="width: 175px;" value="<?= $is_edit
                    ? $cliente->bairro
                    : "" ?>"><br>
                
                <label>Address:</label> <input type="text" name="endereco" id="endereco" style="width: 535px;" value="<?= $is_edit
                    ? $cliente->endereco
                    : "" ?>">
            </fieldset>
            ...
            <fieldset>
                ...
                <div class="grid">
                    <div>
                        <label>Maker/Brand:</label> <input type="text" name="carro_fabricante" value="<?= $is_edit
                            ? $cliente->carro_fabricante
                            : "" ?>"><br>
                        <label>Model:</label> <input type="text" name="carro_modelo" value="<?= $is_edit
                            ? $cliente->carro_modelo
                            : "" ?>"><br>
                        <label>Year:</label> <input type="number" name="carro_ano" style="width: 80px;" value="<?= $is_edit
                            ? $cliente->carro_ano
                            : "" ?>"><br>
                        <label>Chassis ID:</label> <input type="text" name="carro_chassi" value="<?= $is_edit
                            ? $cliente->carro_chassi
                            : "" ?>"><br>
                        <label>Engine:</label> <input type="text" name="carro_motor" value="<?= $is_edit
                            ? $cliente->carro_motor
                            : "" ?>">
                    </div>
                    <div>
                        ...
                        <select name="carro_cambio" style="width: 155px;">
                            <option value="Manual" <?= $is_edit &&
                            $cliente->carro_cambio == "Manual"
                                ? "selected"
                                : "" ?>>Manual</option>
                            <option value="Automatic" <?= $is_edit &&
                            $cliente->carro_cambio == "Automatic"
                                ? "selected"
                                : "" ?>>Automatic</option>
                        </select><br>
                        <label>Induction:</label>
                        <select name="carro_turbo" style="width: 155px;">
                            <option value="Aspirado" <?= $is_edit &&
                            $cliente->carro_turbo == "Aspirado"
                                ? "selected"
                                : "" ?>>Aspirado</option>
                            <option value="Turbo" <?= $is_edit &&
                            $cliente->carro_turbo == "Turbo"
                                ? "selected"
                                : "" ?>>Turbo</option>
                            <option value="Supercharger" <?= $is_edit &&
                            $cliente->carro_turbo == "Supercharger"
                                ? "selected"
                                : "" ?>>Supercharger</option>
                        </select><br>
                        <label>ECU/System:</label> <input type="text" name="carro_ecu" value="<?= $is_edit
                            ? $cliente->carro_ecu
                            : "" ?>"><br>
                        <label>Suspension:</label> <input type="text" name="carro_suspensao" value="<?= $is_edit
                            ? $cliente->carro_suspensao
                            : "" ?>"><br>
                        <label>Wheels/Tires:</label> <input type="text" name="carro_rodas" value="<?= $is_edit
                            ? $cliente->carro_rodas
                            : "" ?>">
                    </div>
                </div>
            ...
            <fieldset>
                ...
                <input type="checkbox" name="uso_drift" id="d" value="1" <?= $is_edit &&
                $cliente->uso_drift
                    ? "checked"
                    : "" ?>> <label for="d" style="width: auto;">Drift</label> &nbsp;&nbsp;
                <input type="checkbox" name="uso_track_day" id="t" value="1" <?= $is_edit &&
                $cliente->uso_track_day
                    ? "checked"
                    : "" ?>> <label for="t" style="width: auto;">Track Day</label> &nbsp;&nbsp;
                <input type="checkbox" name="uso_rua" id="r" value="1" <?= $is_edit &&
                $cliente->uso_rua
                    ? "checked"
                    : "" ?>> <label for="r" style="width: auto;">Street</label> &nbsp;&nbsp;
                <input type="checkbox" name="uso_competicao" id="c" value="1" <?= $is_edit &&
                $cliente->uso_competicao
                    ? "checked"
                    : "" ?>> <label for="c" style="width: auto;">Competition</label>
                <br><br>
                <textarea name="observacoes" rows="4" style="width: 98%;" placeholder="Additional technical notes..."><?= $is_edit
                    ? $cliente->observacoes
                    : "" ?></textarea>
            </fieldset>

            <div style="text-align: right; padding-top: 10px;">
                <button type="button" class="btn-win" onclick="window.location.href='<?= site_url(
                    "clientes"
                ) ?>'">Cancel</button>
                <button type="submit" class="btn-win"><?= $is_edit
                    ? "[ UPDATE_RECORD ]"
                    : "[ REGISTER_ENTRY ]" ?></button>
            </div>
        </form>
    </div>
</div>
