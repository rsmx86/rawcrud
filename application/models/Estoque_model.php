...
class Estoque_model extends CI_Model
{
    ...
    {
        $this->db->select(
            "cp.codigo_sku, cp.nome_produto, e.rua, e.posicao, e.lote, SUM(e.quantidade) as saldo_disponivel, e.status_posicao"
        );
    ...
    {
        $this->db->select(
            "ri.*, cp.nome_produto, cp.codigo_sku, r.codigo_despacho"
        );
    ...
    public function alocar_no_estoque(
        $dados_estoque,
        $id_invoice_item,
        $qtd_alocada
    ) {
        ...
        $this->db->set(
            "quantidade_alocada",
            "quantidade_alocada + " . (float) $qtd_alocada,
            false
        );
    ...
    public function verificar_posicao_ocupada(
        $rua,
        $posicao,
        $id_catalogo,
        $lote
    ...
    public function get_estoque_disponivel()
    {
        //
        $this->db->select(
            "id, nome_produto, valor_unitario as valor_venda, codigo_sku as sku"
        );
    ...
    public function estornar_itens_requisicao($id_requisicao)
    {
        ...
        $itens = $this->db
            ->get_where("requisicao_itens", ["id_requisicao" => $id_requisicao])
            ->result();
    ...
    public function atualizar_produto($id, $dados)
    {
        ...
        if ($this->db->update("catalogo_produtos", $dados)) {
            $this->registrar_log(
                "Editou produto",
                "catalogo_produtos",
                "ID: $id | Novo SKU: " . $dados["codigo_sku"]
            );
            return true;
        }
        return false;
